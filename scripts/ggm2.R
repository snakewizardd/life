...
mu_est <- list(as.numeric(data_all[init_idx[1], ]), as.numeric(data_all[init_idx[2], ]))
Sigma_est <- list(cov(data_all), cov(data_all))
pi_est <- rep(1/K, K)

# Number of EM iterations - increase this for more evolution
...
for (iter in 1:n_iter) {
  ...
  p <- ggplot(data_plot, aes(x = x, y = y, color = cluster)) +
    ...
    annotate("text", x = mu_est[[1]][1], y = mu_est[[1]][2], 
             label = paste("Mean 1: (", round(mu_est[[1]][1], 2), ", ", round(mu_est[[1]][2], 2), ")", sep = ""), 
             color = "black", size = 4, hjust = -0.1, fontface = "bold") +
    annotate("text", x = mu_est[[2]][1], y = mu_est[[2]][2], 
             label = paste("Mean 2: (", round(mu_est[[2]][1], 2), ", ", round(mu_est[[2]][2], 2), ")", sep = ""), 
             color = "black", size = 4, hjust = -0.1, fontface = "bold") +
    labs(title = sprintf("EM Algorithm for Gaussian Mixture Model: Iteration %d", iter),
         subtitle = sprintf("Log-likelihood: %.2f", loglik),
         x = "X", y = "Y", color = "Cluster", linetype = "Component") +
  ...
  ggsave(filename, plot = p, width = 8, height = 6, dpi = 150)
  filenames[iter] <- filename
}

# Combine all frames into an animated GIF
gifski(png_files = filenames, gif_file = "./outputs/em_gmm_evolution4.gif", width = 900, height = 750, delay = 0.5)
